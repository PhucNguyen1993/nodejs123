...
//    );
        ...
        const createPatientFn = new lambda.Function(this, 'CreatePatientFunction', {
          ...
          role: role,
          layers: [
            lambda.LayerVersion.fromLayerVersionArn(this, 'Psycopg2Layer', 'arn:aws:lambda:us-east-1:705762609683:layer:MyLayer:1')
          ]
        });

        const psycopg2Layer = new lambda.LayerVersion(this, 'Psycopg2Layer', {
          code: lambda.Code.fromAsset('layers/psycopg2-layer.zip'),
       ...
       const dbInstance = new rds.DatabaseInstance(this, 'MyDatabase', {
         ...
         vpc,
         instanceType: ec2.InstanceType.of(ec2.InstanceClass.T3, ec2.InstanceSize.MICRO),
         credentials: rds.Credentials.fromGeneratedSecret('postgres'), // user: postgres
         databaseName: 'hospital', // tên database
         multiAz: false,
         allocatedStorage: 20,
       ...
       createPatientFn.addEnvironment('DB_SECRET', dbInstance.secret.secretName);
       createPatientFn.addEnvironment('DB_HOST', dbInstance.dbInstanceEndpointAddress);
       createPatientFn.addEnvironment('DB_NAME', 'hospital');
       createPatientFn.addEnvironment('DB_PORT', '5432');
